...
<?php
           ...
           echo '<form action="ApplicationView.php" method="post" id="form1">';
                ...
                echo'   </a><br><br><a href="ApplicationView.php?a=true" id="linkMenu">Not Complete Applications</a><br><br>
                    ...
                    <a href="ApplicationView.php?c=true" id="linkMenu">Approved Applications';echo  $_SESSION['NRcount2'];echo '</a> <br><br>
     ...
     //if connected then Select Database.
      $db=mysqli_select_db($conn,"dspdb");
              ...
              $body="Dear ".$nameWithInitials.",<br><br>".$reason."<br><br>Please send those details as a reply to this email or mention your NIC number and the reason on the mail subject (Ex: 000000000v Send to nic copy)<br><br>Thank You!<br>Manager<br>(DSP GROUP MICRO CREDITS)";
              //echo $body."<br>";
  ...
  function sendmail($body,$subject,$to){
    ...
    if(!$mail->Send()){
       ...
       require_once("connection.php");
       myUpdateFunction($conn);

      }
  }

  function myUpdateFunction($conn){
                
                  $sql5="update customer SET loanStatus ='W' where customerId='{$_SESSION['cId']}'";
                  if ($conn->query($sql5) === TRUE) {
                      
                      echo "<script>alert('Email send successfully and system updated');</script>";
                  } else {
                      echo "<script>alert('System not updated');</script>" ;
                  }
